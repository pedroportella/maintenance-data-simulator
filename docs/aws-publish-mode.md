...
simulator publish-aws --scenario baseline-week --repeat 25 --confirm-aws-publish
```
...
Use `--repeat` to publish a larger deterministic synthetic pack for queue and worker volume checks. Each repeated copy has unique synthetic source ids and idempotency keys, so it exercises the queue with additional records while remaining replay-safe for the same seed and repeat value. To add more new messages later, increase `--repeat` or use a different `--seed`.
